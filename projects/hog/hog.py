...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    ini_num = strategy(0, 0)
    for i in range(goal):
        for j in range(goal):
            if strategy(i, j) != ini_num:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    num = 0
    score = 0
    avg_fun = make_averaged(roll_dice, times_called)
    for i in range(1, 11):
        avg_score = avg_fun(i, dice)
        if score < avg_score:
            score = avg_score
            num = i
...
def final_strategy(score, opponent_score):
    ...
    When it would give me more points to roll 0,return 0.
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if sus_strategy(score, opponent_score) >= sus_update(6, score, opponent_score):
        return 0
    else:
        return 6
    # END PROBLEM 12
